// variant4.cpp: Divisibility testing with threads, wait until all done then print
#include <iostream>
#include <fstream>
#include <vector>
#include <thread>
...
int main() {
    ...
    for (long n = 2; n <= limit; ++n) {
        if (is_prime_threaded(n, num_threads)) {
            ...
            all_primes.push_back({n, 0, ts});  // 0 for main thread
        }
    }
    ...
    for (const auto& p : all_primes) {
        lock_guard<mutex> lock(print_mutex);
        cout << "Thread main at " << p.timestamp << " found " << p.prime << endl;
    }
